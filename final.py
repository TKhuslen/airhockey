...
def gameLoop():
    ...
    while not gameExit:
        ...
        if (disc.x <= disc.width and (disc.y <= screen.get_height() / 2 + goalheight) and (
                disc.y >= screen.get_height() - goalheight)):
            score2 += 1
            serveDirection = -1
            resetPuck()
        elif (disc.x >= screen.get_width() - goalwidth - disc.width) and (
                disc.y <= screen.get_height() / 2 + goalheight) and (disc.y >= screen.get_height() / 2 - goalheight):
            score1 += 1
            serveDirection = 1
            resetPuck()
